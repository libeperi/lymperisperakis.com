    ...
    <div className="prose-chat">
      <ReactMarkdown
        ...
        components={{
          ...
          code: ({ children }) => (
            <code className="font-mono text-[13px] bg-white/[0.06] px-1.5 py-0.5 rounded-md">
              {children}
            </code>
          ),
          pre: ({ children }) => (
            <pre className="font-mono text-[13px] bg-white/[0.04] border border-white/[0.08] p-3 rounded-xl overflow-x-auto my-2 leading-relaxed">
              {children}
            </pre>
          ),
          a: ({ children, href }) => (
          ...
          h1: ({ children }) => (
            <h3 className="font-display text-lg font-semibold tracking-tight text-ink mb-2 mt-1">
              {children}
            </h3>
          ),
          h2: ({ children }) => (
          ...
          h3: ({ children }) => (
            <h4 className="font-display text-base font-semibold tracking-tight text-ink mb-1.5 mt-1">
              {children}
            </h4>
          ),
          blockquote: ({ children }) => (
      ...
      {hasOpenThink && streaming && visible && (
        <span className="block mt-1 font-mono text-[10px] uppercase tracking-[0.2em] text-ink-faint">
          thinking…
        </span>
      )}
    </div>
